module.exports = {
  'Test Home PlaylistControls': (browser) => {
    browser
      .url(process.env.VUE_DEV_SERVER_URL)
      .waitForElementVisible('#playButton', 5000)
      .assert.elementPresent('#pauseButton')
      .click('#playButton')
      // after waiting for 1.5 seconds, assert playback timer displays "1" to represent the number of seconds we are into the song
      .pause(1500)
      .assert.containsText('#currentTimeInSeconds', 1)
      // pause the song, wait 3 seconds, then assert playback timer still displays the value set at pause
      .click('#pauseButton')
      .pause(3000)
      .assert.containsText('#currentTimeInSeconds', 1, 'Playback timer is still "1" after pausing and waiting for 3 seconds.')
      // resume playback, wait 2 seconds, assert timer indicates we are now 3 total seconds into the track
      .click('#playButton')
      .pause(2000)
